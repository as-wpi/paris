...
def test_regime_runs_and_conditional_table(fcntx, funds, spx, rf_series):
    ...
    np.testing.assert_allclose(t["own mean (ann.)"], paris.momentum_state_table(fcntx)["own mean (ann.)"])
    ...
    np.testing.assert_allclose(rel["active mean (ann.)"], paris.momentum_state_table(fcntx, basis="relative")["own mean (ann.)"])
